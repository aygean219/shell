...
//procesului copil, iar procesul copil verifică şi afişează dacă acest 
...
int main(int ardc,char* argv[])
{
	...
	{	//inchid capatul de scriere din pipe
		...
		while(n>0)
		{
			...
			if(n%2==0)
			{
				printf("IN CHILD --->Numarul citit %d este un numar par\n",n);
			}
			else
			{
				printf("IN CHILD --->Numarul citit %d este un numar impar\n",n);
			}
		}
		close(pd[0]);
		exit(EXIT_SUCCESS);
